...
class RwsCheck:
    ...
    def check_well_known_list(self, field, list1, list2):
        ...
        return [
            f"Encountered an inequality between the PR submission and the {WELL_KNOWN} file:\n"
        ...
        for primary, curr_rws_set in check_sets.items():
            ...
            try:
                ...
                if well_known_set.primary != curr_rws_set.primary:
                    self.error_list.append(
                        f"The {WELL_KNOWN} set's primary ({well_known_set.primary}) did not equal "
                        + f"the PR set's primary ({curr_rws_set.primary})"
                    )
                self.error_list.extend(
                    self.check_well_known_list(
                        "associatedSites",
    ...
    def find_invalid_removal(self, subtracted_sets):
        ...
        for primary in subtracted_sets:
            ...
            try:
                ...
                self.error_list.append(
                    f"Unexpected error when accessing {url}; Received error: {inst}"
                )
    ...
    def find_invalid_alias_eSLDs(self, check_sets):
        ...
        for primary, curr_set in check_sets.items():
            for aliased_site, aliases in curr_set.ccTLDs.items():
                ...
                aliased_eSLD, aliased_tld = (
                    aliased_site.split(".")[0],
                ...
                variants = [
                    (site, site.split(".")[0], site.split(".")[-1]) for site in aliases
                ]
            ...
            for service_site in curr_set.service_sites:
                try:
                    ...
                    if "X-Robots-Tag" not in r_service.headers:
                        self.error_list.append(
                            f"The service site {service_site} does not have an X-Robots-Tag in its "
                            + "header"
                        )
                    else:
                        ...
                        if ":" in robots_tag:
                            self.error_list.append(
                                f"The service site {service_site} contains an 'X-Robots-Tag' "
                                + "that does not meet the policy requirements"
                            )
                        elif "none" not in robots_tag and "noindex" not in robots_tag:
                            self.error_list.append(
                                f"The service site {service_site} does not have a "
